...
static int ring_buffer_write(RingBufferContext* ctx, uint8_t* buffer, int size)
{
...
static int ring_buffer_read(RingBufferContext* ctx, uint8_t* buffer, int size)
{
...
static int ring_buffer_init(RingBufferContext* ctx, int size)
{
...
static void ring_buffer_free(RingBufferContext* ctx)
{
	free(ctx->buffer_base);
	ctx->buffer_base = NULL;
}

static int ring_buffer_datasize(RingBufferContext* ctx)
{
	return ctx->data_size;
}

static int ring_buffer_buffersize(RingBufferContext* ctx)
{
	return ctx->buffer_size;
}

static int ring_buffer_freesize(RingBufferContext* ctx)
{
...
int CFfmpeg::Open(const char* pszUrl)
{
	...
	else
	{
		...
		//ffio_init_context(&outputpb, outputbuffer, MAIN_BUFFER_SIZE, 1, &outputringbuffer, NULL, write_data, NULL );
		oc->pb = &outputpb;
